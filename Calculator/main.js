...
function Input_Digit(digit) {
  ...
  if (Wait_Second_Operand === true) {
    ...
    // This overwrites Display_Value if the current values is 0 otherwise it adds to it
...
function Handle_Operator(Next_Operator) {
  ...
  // When an operator key is pressed we convert the number displayed on the screen to a number and then store the result in Calculator.First_Operand if it doesn't already exist
...
function Update_Display() {
  // Makes use of the calculator-screen class to target the input tage in the HTML document
...
keys.addEventListener("click", (event) => {
  ...
  const { target } = event;
  // If the elemtn that was clicked on is not a button, exit the function
